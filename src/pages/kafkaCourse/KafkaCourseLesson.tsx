...
const KafkaCourseLesson: React.FC = () => {
  ...
  usePageTitle(lesson?.title ?? 'Kafka Course');
  ...
  const handleNext = () => {
    if (nextLesson) {
      ...
      navigate(`/kafka-course/lesson/${nextLesson.id}`);
    }
  };
  ...
  if (!lesson) {
    return (
      <Box p={4}>
        <Button startIcon={<ArrowBackIcon />} onClick={() => navigate('/kafka-course')}>
          Back to Course
        </Button>
                ...
                <Box sx={{ pl: 1 }}>
                  {phaseLessons.map((l) => {
                    ...
                    return (
                      <Box
                        key={l.id}
                        onClick={() => { navigate(`/kafka-course/lesson/${l.id}`); if (isMobile) setSidebarOpen(false); }}
                        sx={{
                          display: 'flex',
      ...
      >
        ...
        <Box display="flex" alignItems="center" gap={1} mb={2.5}>
          ...
          <Typography variant="caption" color="text.disabled" sx={{ '& span': { color: 'text.secondary' } }}>
            <span>Apache Kafka Course</span> › <span>Phase {lesson.phase}</span> › Lesson {lesson.lesson}
          </Typography>
          <Box sx={{ ml: 'auto' }}>
        ...
        <Box display="flex" justifyContent="space-between" gap={2} flexWrap="wrap" pb={4}>
          <Button
            variant="outlined"
            startIcon={<ArrowBackIcon />}
            onClick={() => prevLesson && navigate(`/kafka-course/lesson/${prevLesson.id}`)}
            disabled={!prevLesson}
          ...
          <Button
            ...
            onClick={nextLesson ? handleNext : () => navigate('/kafka-course')}
            sx={{
              fontWeight: 700,
              borderRadius: 2,
...
export default KafkaCourseLesson;
