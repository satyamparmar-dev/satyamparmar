    ...
    <Box
      ...
      sx={{
        ...
        '& pre': {
          ...
          overflow: 'auto',
          my: 1.5,
          '& code': { fontFamily: 'JetBrains Mono, Fira Code, monospace', fontSize: '0.82rem', lineHeight: 1.7 },
        },
        '& code.inline-code': {
          bgcolor: 'action.hover', px: 0.75, py: 0.25, borderRadius: 0.75,
...
const SectionCard: React.FC<{ section: CourseSection }> = ({ section }) => {
  ...
  return (
    <Paper
      elevation={0}
      sx={{
        mb: 2, borderRadius: 2.5, border: '1px solid',
    ...
    >
      <Box
        sx={{
          ...
          px: 2, py: 1.25, borderBottom: '1px solid', borderColor: alpha(cfg.borderColor, 0.15),
        }}
      >
        <Box sx={{ color: cfg.headerColor, display: 'flex' }}>{cfg.icon}</Box>
        <Typography variant="overline" sx={{ color: cfg.headerColor, fontWeight: 700, fontSize: '0.62rem', letterSpacing: '0.08em' }}>
          {cfg.label}
        </Typography>
...
const JavaCourseLesson: React.FC = () => {
  ...
  usePageTitle(lesson?.title ?? 'Java Course');
  ...
  const handleNext = () => {
    if (nextLesson) {
      ...
      navigate(`/java-course/lesson/${nextLesson.id}`);
    }
  };
  ...
  if (!lesson) {
    return (
      <Box p={4}>
        <Button startIcon={<ArrowBackIcon />} onClick={() => navigate('/java-course')}>Back to Course</Button>
              ...
              >
                ...
                <Box flex={1} minWidth={0}>
                  ...
                  <LinearProgress
                    ...
                    value={phasePct}
                    sx={{
                      height: 2, borderRadius: 1, mt: 0.25,
                      bgcolor: alpha(p.color, 0.15),
                      '& .MuiLinearProgress-bar': { bgcolor: p.color },
                    }}
                  />
                </Box>
                <Typography variant="caption" sx={{ color: p.color, fontWeight: 700, fontSize: '0.6rem', flexShrink: 0 }}>
                  {phasePct}%
                </Typography>
                {isExpanded
                ...
                <Box sx={{ pl: 1 }}>
                  {phaseLessons.map((l) => {
                    ...
                    return (
                      <Box
                        key={l.id}
                        onClick={() => { navigate(`/java-course/lesson/${l.id}`); if (isMobile) setSidebarOpen(false); }}
                        sx={{
                        ...
                        {isDone
                          ...
                          : (
                            <Box sx={{
                              ...
                              borderColor: isCurrent ? p.color : 'divider', mt: 0.2, flexShrink: 0,
                            }} />
                          )
                        }
                        <Box flex={1} minWidth={0}>
                          <Typography
                            variant="caption"
                            sx={{
                              display: 'block', fontWeight: isCurrent ? 700 : 400,
      ...
      <Box ref={contentRef} sx={{ flex: 1, overflow: 'auto', px: { xs: 2, sm: 3 }, py: 2.5 }}>
        ...
        <Box display="flex" alignItems="center" gap={1} mb={2.5}>
          ...
          <Typography variant="caption" color="text.disabled" sx={{ '& span': { color: 'text.secondary' } }}>
            <span>Java Course</span> › <span>Phase {lesson.phase}</span> › Lesson {lesson.lesson}
          </Typography>
        </Box>
        ...
        <Paper
          elevation={0}
          sx={{
            mb: 2.5, p: { xs: 2, sm: 2.5 }, borderRadius: 2.5, border: '1px solid', borderColor: 'divider',
            background: phase
              ? isDark
        ...
        >
          <Box display="flex" alignItems="flex-start" justifyContent="space-between" gap={2} flexWrap="wrap">
            <Box flex={1} minWidth={0}>
              <Box display="flex" gap={1} flexWrap="wrap" mb={1}>
                {phase && (
                  <Chip
                    ...
                    sx={{ bgcolor: alpha(phase.color, 0.12), color: phase.color, fontWeight: 700, fontSize: '0.65rem', height: 22 }}
                  />
                )}
                <Chip
                  size="small"
              ...
              <Typography variant="h5" fontWeight={800} sx={{ letterSpacing: '-0.02em', lineHeight: 1.3, mb: 0.5 }}>
                {lesson.title}
              </Typography>
            ...
            <Button
              ...
              sx={{
                fontWeight: 700, borderRadius: 2, flexShrink: 0, fontSize: '0.75rem',
                ...(isComplete
        ...
        {lesson.tags.length > 0 && (
          <Box display="flex" gap={0.75} flexWrap="wrap" mb={2}>
            {lesson.tags.map((tag) => (
              <Chip key={tag} label={tag} size="small" sx={{ height: 20, fontSize: '0.62rem', fontWeight: 500 }} />
            ))}
          </Box>
        )}
        ...
        {lesson.checkpoints.length > 0 && (
          <Paper
            elevation={0}
            sx={{
              mb: 2.5, p: 2.5, borderRadius: 2.5, border: '1px solid',
          ...
          >
            ...
            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1.5 }}>
              Say each answer out loud before moving to the next lesson. If you struggle, re-read the relevant section above.
            </Typography>
            <Box component="ol" sx={{ m: 0, pl: 2.5 }}>
              {lesson.checkpoints.map((q, i) => (
                <Box component="li" key={i} sx={{ mb: 1, fontSize: '0.88rem', lineHeight: 1.6, color: 'text.primary', fontWeight: 500 }}>
                  {q}
                </Box>
              ))}
            </Box>
        ...
        <Box display="flex" justifyContent="space-between" gap={2} flexWrap="wrap" pb={4}>
          <Button
            variant="outlined"
            startIcon={<ArrowBackIcon />}
            onClick={() => prevLesson && navigate(`/java-course/lesson/${prevLesson.id}`)}
            disabled={!prevLesson}
          ...
          <Button
            ...
            onClick={nextLesson ? handleNext : () => navigate('/java-course')}
            sx={{
              fontWeight: 700, borderRadius: 2,
              bgcolor: phase?.color ?? '#f59e0b',
              '&:hover': { filter: 'brightness(0.9)', bgcolor: phase?.color ?? '#f59e0b' },
            }}
          >
            {nextLesson ? nextLesson.title : 'Finish Course'}
          </Button>
        </Box>
      </Box>
    </Box>
...
export default JavaCourseLesson;
