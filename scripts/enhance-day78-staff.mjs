...
import { buildDay78Sections } from './day78-builder.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const filePath = join(__dirname, '..', 'public', 'data', 'days', 'phase9-day78.json');
...
function verify(json) {
  ...
  return {
    ...
    hasFresherEx: !!fresherEx && /Day78FresherExercise/.test(fresherEx.solution || ''),
    hasStaffEx: !!staffEx && /Day78StaffExercise/.test(staffEx.solution || ''),
...
function bandCounts(theoryContent) {
  ...
  const fresher = L.filter((l) =>
    /### (Plain-language|What is|Your first|When you sketch|How to read)/i.test(l)
  ).length;
  const senior = L.filter((l) =>
    /### (How |Comparison table|Step sequence|Common mistakes|When Singleton)/i.test(l)
  ).length;
...
console.log('Patches applied:', patched ? 'full day 78 rebuild' : '(none)');
