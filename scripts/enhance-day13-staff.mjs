...
import {
  ...
  WRONG_ANSWERS,
  CHEATSHEET_CONTENT,
} from "./day13-staff-bundle.mjs";
import { CONCEPTUAL, CODE_BASED, SENIOR_SCENARIO } from "./day13-interview-data.mjs";

const FILE = "public/data/days/phase2-day13.json";
...
function applySelectivePatches(day) {
  ...
  if (wordCount(why.content) < 600 || paragraphCount(why.content) < 5 || paragraphCount(why.content) > 6) {
    why.title = "Why nested types become heap incidents, shade failures, and interview traps";
  ...
  if (
    ...
    theory.title = "Theory and Internals — nested bytecode, NestHost, lambdas, and retention";
  ...
  if (
    ...
    Object.assign(basic, {
      title: "Basic — nested layout table, javap/jcmd reference, failure modes, CI cues",
      description:
        "Static **println**-only reference for **Outer$Inner** names, **this$0**, **NestHost**, and **shade** failure signatures.",
      code: BASIC_CODE,
      output: BASIC_OUTPUT,
      filename: "Day13Basic.java",
    });
  }
  ...
  if (
    ...
    Object.assign(inter, {
      title: "Intermediate — four scenarios: inner retention, shade split, reflection, lambda indy",
      description:
        "Each scenario names **javap**, **jcmd**, **jdeps**, or **jstack** because **nested** bugs masquerade as framework leaks.",
      code: INTERMEDIATE_CODE,
      output: INTERMEDIATE_OUTPUT,
      filename: "Day13Intermediate.java",
    });
  }
  ...
  if (
    ...
    Object.assign(adv, {
      title: "Advanced — nest pattern catalog, leak weights, refactor checklist",
      description:
        "Deterministic **Map**/**List**/**record** model mirroring Staff guardrails for **nested** **callback** retention reviews.",
      code: ADVANCED_CODE,
      output: ADVANCED_OUTPUT,
      filename: "Day13Advanced.java",
    });
  }

  if ((pit.items || []).length !== 8) {
    pit.title = "Common Pitfalls — nested and inner classes";
    pit.items = PITFALLS;
  }
  ...
  if (
    ...
    ex.title = "Exercise — Hull static nested builder and local-class demo";
  ...
  if (
    ...
    iv.title = "Interview Drill — Nested and Inner Classes";
  ...
  if (cheatsheetRows(cs.content) < 10) {
    cs.title = "Cheatsheet — nested vs inner bytecode, retention, and tooling";
