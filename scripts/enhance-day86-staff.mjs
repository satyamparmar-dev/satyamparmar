...
import { buildDay86Sections } from './day86-builder.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const filePath = join(__dirname, '..', 'public', 'data', 'days', 'phase10-day86.json');
...
function verify(json) {
  ...
  return {
    ...
    hasFresherEx: !!fresherEx && /Day86FresherExercise/.test(fresherEx.solution || ''),
    hasStaffEx: !!staffEx && /Day86StaffExercise/.test(staffEx.solution || ''),
...
function bandCounts(theoryContent) {
  ...
  const senior = L.filter((l) =>
    /### (How Mockito|Surefire|Comparison|Step sequence|Common mistakes|Bytecode)/i.test(l)
  ).length;
  const tl = L.filter((l) => /### (Choosing|Code review|Stakeholder|Trade-off)/i.test(l)).length;
  const staff = L.filter((l) =>
    /### (Byte Buddy|jcmd|Java 8|Architecture guard|What breaks at scale)/i.test(l)
  ).length;
...
console.log('Patches applied:', patched ? 'full day 86 rebuild' : '(none)');
