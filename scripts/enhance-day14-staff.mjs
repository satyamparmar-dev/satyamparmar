...
import {
  ...
  WRONG_ANSWERS,
  CHEATSHEET_CONTENT,
} from "./day14-staff-bundle.mjs";
import { CONCEPTUAL, CODE_BASED, SENIOR_SCENARIO } from "./day14-interview-data.mjs";

const FILE = "public/data/days/phase2-day14.json";
...
function conceptualFollowUpsTooShort(iv) {
  for (const q of iv.conceptual || []) {
    for (const f of q.followUps || []) {
      if (wordCount(f.answer || "") < 60) return true;
    }
  }
  return false;
}

function codeBasedFails(iv) {
  const arr = iv.codeBased || [];
  if (arr.length < 8) return true;
  for (const q of arr) {
    if (wordCount(q.answer || "") < 60) return true;
    if (!String(q.question || "").includes("```java")) return true;
  }
  return false;
}
...
function applySelectivePatches(day) {
  ...
  if (wordCount(why.content) < 600 || paragraphCount(why.content) < 5 || paragraphCount(why.content) > 6) {
    why.title = "Why collection contracts decide CME incidents, cache truth, and GC storms";
  ...
  if (
    ...
    theory.title = "Theory and Internals — ArrayList, HashMap, concurrency, and immutable snapshots";
  ...
  if (
    ...
    Object.assign(basic, {
      title: "Basic — collection matrix, jcmd/jstack reference, failure modes, CI cues",
      description:
        "Static **println**-only reference for **List**/**Map** choices, **modCount**, **ConcurrentHashMap**, and **copyOf** guardrails.",
      code: BASIC_CODE,
      output: BASIC_OUTPUT,
      filename: "Day14Basic.java",
    });
  }
  ...
  if (
    ...
    Object.assign(inter, {
      title: "Intermediate — four scenarios: CME JSON walk, mutable keys, resize storm, synchronizedMap myth",
      description:
        "Each scenario names **jcmd**, **jstack**, **jmap**, **jstat**, or **javap** because **collection** bugs masquerade as framework faults.",
      code: INTERMEDIATE_CODE,
      output: INTERMEDIATE_OUTPUT,
      filename: "Day14Intermediate.java",
    });
  }
  ...
  if (
    ...
    Object.assign(adv, {
      title: "Advanced — workload picker, risk weights, collection PR checklist",
      description:
        "Deterministic **Map**/**List**/**record** model mirroring Staff guardrails for **collection** **architecture** reviews.",
      code: ADVANCED_CODE,
      output: ADVANCED_OUTPUT,
      filename: "Day14Advanced.java",
    });
  }

  if ((pit.items || []).length !== 8) {
    pit.title = "Common Pitfalls — Java Collections Framework";
    pit.items = PITFALLS;
  }
  ...
  if (
    ...
    ex.title = "Exercise — TokenHistogram merge with immutable snapshot API";
  ...
  if (
    conceptualFails(iv) ||
    conceptualFollowUpsTooShort(iv) ||
    codeBasedFails(iv) ||
    seniorFails(iv) ||
    (iv.wrongAnswers || []).length !== 8 ||
    !iv.jobSwitch
  ) {
    iv.title = "Interview Drill — Java Collections Framework";
    if (conceptualFails(iv) || conceptualFollowUpsTooShort(iv)) iv.conceptual = CONCEPTUAL;
    if (codeBasedFails(iv)) iv.codeBased = CODE_BASED;
  ...
  if (cheatsheetRows(cs.content) < 10) {
    cs.title = "Cheatsheet — collections, concurrency, and JVM diagnostics";
