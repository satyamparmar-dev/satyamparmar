...
import {
  ...
  WRONG_ANSWERS,
  CHEATSHEET_CONTENT,
} from "./day12-staff-bundle.mjs";
import { CONCEPTUAL, CODE_BASED, SENIOR_SCENARIO } from "./day12-interview-data.mjs";

const FILE = "public/data/days/phase2-day12.json";
...
function applySelectivePatches(day) {
  ...
  if (wordCount(why.content) < 600 || paragraphCount(why.content) < 5 || paragraphCount(why.content) > 6) {
    why.title = "Why encapsulation and immutability decide invariants, aliases, and cache truth";
  ...
  if (
    ...
    theory.title = "Theory and Internals — private fields, defensive copies, records, JMM, and Staff tooling";
  ...
  if (
    ...
    Object.assign(basic, {
      title: "Basic — encapsulation matrix, javap/jcmd reference, failure signatures, CI guardrails",
      description:
        "Static **println**-only reference pairing **private** **heap** **fields** with **List.copyOf** vocabulary for **DTO** reviews.",
      code: BASIC_CODE,
      output: BASIC_OUTPUT,
      filename: "Day12Basic.java",
    });
  }
  ...
  if (
    ...
    Object.assign(inter, {
      title: "Intermediate — four scenarios: leaked List, Date key, shallow record, racy publication",
      description:
        "Each scenario names **javap**, **jmap**, or **jstack** because **alias** bugs masquerade as serialization or cache mysteries.",
      code: INTERMEDIATE_CODE,
      output: INTERMEDIATE_OUTPUT,
      filename: "Day12Intermediate.java",
    });
  }
  ...
  if (
    ...
    Object.assign(adv, {
      title: "Advanced — alias risk scoreboard, immutability checklist, publication posture matrix",
      description:
        "Deterministic **Map**/**List**/**record** model mirroring Staff guardrails for **DTO** graphs under **Kafka** load.",
      code: ADVANCED_CODE,
      output: ADVANCED_OUTPUT,
      filename: "Day12Advanced.java",
    });
  }

  if ((pit.items || []).length !== 8) {
    pit.title = "Common Pitfalls — encapsulation and immutability";
    pit.items = PITFALLS;
  }
  ...
  if (
    ...
    ex.title = "Exercise — PositionSnapshot with List.copyOf and alias proof";
  ...
  if (
    ...
    (iv.codeBased || []).length < 8 ||
    seniorFails(iv) ||
    (iv.wrongAnswers || []).length !== 8 ||
    !iv.jobSwitch
  ) {
    iv.title = "Interview Drill — Encapsulation and Immutability";
  ...
  if (cheatsheetRows(cs.content) < 10) {
    cs.title = "Cheatsheet — encapsulation and immutability";
