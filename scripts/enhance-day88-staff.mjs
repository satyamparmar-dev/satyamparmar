...
import { buildDay88Sections } from './day88-builder.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const filePath = join(__dirname, '..', 'public', 'data', 'days', 'phase10-day88.json');
...
function verify(json) {
  ...
  return {
    ...
    hasFresherEx: !!fresherEx && /Day88FresherExercise/.test(fresherEx.solution || ''),
    hasStaffEx: !!staffEx && /Day88StaffExercise/.test(staffEx.solution || ''),
...
function bandCounts(theoryContent) {
  ...
  const fresher = L.filter((l) =>
    /### (Plain-language|What is|Your first|When you click|How to read)/i.test(l)
  ).length;
  const senior = L.filter((l) =>
    /### (How Mockito|Comparison table|Numbered sequence|Step sequence|Common mistakes|Surefire|Bytecode)/i.test(l)
  ).length;
  const tl = L.filter((l) => /### (Choosing|Code review|Stakeholder|Trade-off|Tool commands)/i.test(l)).length;
  const staff = L.filter((l) =>
    /### (jcmd|Java 8|Architecture guard|What breaks at scale|JVM)/i.test(l)
  ).length;
...
console.log('Patches applied:', patched ? 'full day 88 rebuild' : '(none)');
