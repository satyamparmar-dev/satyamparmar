...
import { buildDay90Sections } from './day90-builder.mjs';
...
function verify(json) {
  ...
  return {
    ...
    hasFresherEx: !!fresherEx && /Day90FresherExercise/.test(fresherEx.solution || ''),
    hasStaffEx: !!staffEx && /Day90StaffExercise/.test(staffEx.solution || ''),
...
function bandCounts(theoryContent) {
  ...
  const fresher = L.filter((l) =>
    /### (Plain-language|What is a full mock|Your first three|Mini Java|How to read when)/i.test(l)
  ).length;
  const senior = L.filter((l) =>
    /### (How silent|Comparison table — mock|Numbered sequence — one afternoon|Common mistakes that look)/i.test(l)
  ).length;
  const tl = L.filter((l) =>
    /### (Choosing what to drill|Code review — what you demand|Stakeholder talk — explain your mock)/i.test(l)
  ).length;
  const staff = L.filter((l) =>
    /### (Tool commands when the mock|jcmd when the mock|Java 8 versus|JVM signals when you code|Architecture guardrail for teams)/i.test(l)
  ).length;
...
console.log('Patches applied:', patched ? 'full day 90 rebuild' : '(none)');
