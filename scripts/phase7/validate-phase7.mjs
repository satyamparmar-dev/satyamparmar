...
function validateDay(n) {
  const file = path.join(daysDir, `phase7-day${n}.json`);
  ...
  const uc = data.sections.find((s) => s.type === "useCases");
  if (!uc?.title || !String(uc.content ?? "").trim()) {
    throw new Error(`day ${n}: useCases requires title and non-empty content`);
  }
  ...
  console.log(`OK phase7-day${n}.json`);
}

const nums = process.argv.slice(2).map(Number).filter((x) => !Number.isNaN(x));
const run = nums.length ? nums : [59, 60, 61, 62, 63, 64, 65, 66, 67];
