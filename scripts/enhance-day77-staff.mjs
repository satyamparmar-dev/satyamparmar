...
import { buildDay77Sections } from './day77-builder.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const filePath = join(__dirname, '..', 'public', 'data', 'days', 'phase9-day77.json');
...
function verify(json) {
  ...
  return {
    ...
    hasFresherEx: !!fresherEx && /Day77FresherExercise/.test(fresherEx.solution || ''),
    hasStaffEx: !!staffEx && /Day77StaffExercise/.test(staffEx.solution || ''),
...
function bandCounts(theoryContent) {
  ...
  const fresher = L.filter((l) =>
    /### (Plain-language|What is SOLID|Your first|When you sketch|How to read)/i.test(l)
  ).length;
  const senior = L.filter((l) =>
    /### (How dependency|Comparison table|Step sequence|Common mistakes|When SRP)/i.test(l)
  ).length;
  const tl = L.filter((l) => /### (Choosing|Code review|How to explain|Stakeholder)/i.test(l)).length;
  const staff = L.filter((l) =>
    /### (What the JVM|jcmd|Java 8|Architecture guard|What breaks at scale|javap)/i.test(l)
  ).length;
...
console.log('Patches applied:', patched ? 'full day 77 rebuild' : '(none)');
